...
public class AsyncHttpClient {
    ...
    /***
     * tanwh modify， 由于在http请执行过程中，httpContext会增加一些属性（比如“"http.protocol.redirect-locations"),
     * 所以不同的请求应该有不同的httpContext,以免上一个请求的参数影响到下一个请求，比如导致多个同样的请求，如果需要重定位，则会导致失败！！
    */
    private final HttpContext mBasicHttpContext; 
    ...
    public AsyncHttpClient(SchemeRegistry schemeRegistry) {
        ...
        HttpProtocolParams.setUserAgent(httpParams, String.format("qqlive-async-http/%s", VERSION));
        ...
        mBasicHttpContext = new SyncBasicHttpContext(new BasicHttpContext());
        httpClient = new DefaultHttpClient(cm, httpParams);
        
        //打开循环重新向，解决评论引发的循环重定向问题：重定向在重试时的location一样，如果不打开则会抛CircularRedirectException引发ClientProtocolException modified by danccyai 2014-06-02
     ...
     * 由于该函数会改变所有请求的cookie，从而导致问题，所以该函数不再实现任何操作
     */
    @Deprecated  
    public void setCookieStore(CookieStore cookieStore) {
        //mBasicHttpContext.setAttribute(ClientContext.COOKIE_STORE, cookieStore);
    }
    ...
    protected RequestHandle sendRequest(DefaultHttpClient client, HttpContext httpContext, HttpUriRequest uriRequest, String contentType, ResponseHandlerInterface responseHandler, Context context) {
        ...
        //tangwh  add, 由于为了允许不同请求的有不同的cookie参数，所以我们的cookie参数没有用setCookieStore（）来设置，而是直接设置到uriRequest的Header参数中了，
        //为了避免在http请求有有2个Cookie头，所以在每次请求前要将cookieStore设置为空
